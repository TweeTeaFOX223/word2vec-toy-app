...
def word2vec_all_words_to_pldf(wordvec_model) -> pl.DataFrame:
    word_data = []
...
def check_word2vec_model_in_words(wordvec_model, target_words):
    exsist_words = []
    non_exsist_words = []
...
def gensim_wordvec_top_simword(
    wordvec_model, positive_words, negative_words
) -> pl.DataFrame:
    all_target_words = []
    ...
    exsist_words, non_exsist_words = check_word2vec_model_in_words(
        wordvec_model, all_target_words
    )
    ...
    if len(non_exsist_words) >= 1:
        return pl.DataFrame(
            data={
                "エラー：モデルの語彙に存在しない単語が入力されました": non_exsist_words
            },
        )

    if positive_words == []:
        most_similar_vec = wordvec_model.most_similar(negative=negative_words, topn=200)
    # ネガティブワードがない場合はポジティブだけ表示
    elif negative_words == []:
        most_similar_vec = wordvec_model.most_similar(positive=positive_words, topn=200)
    else:
...
def gensim_wordvec_two_words_sim(wordvec_model, target_words1, target_words2):
...
if __name__ == "__main__":
    ...
    result_df = gensim_wordvec_top_simword(wordvec_model, sample1_posi, sample1_nega)
    print(result_df)
